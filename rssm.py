...
class SSMBackbone(nn.Module):
    """Simple gated residual transition used for S4/S5/Mamba2 variants."""

    def __init__(self, deter, stoch, act_dim, hidden, act="SiLU", layers=2):
        super().__init__()
        act_cls = getattr(torch.nn, act)
        inp = deter + stoch + act_dim
        blocks = []
        for _ in range(int(layers)):
            blocks.extend(
                [
                    nn.Linear(inp, hidden, bias=True),
                    nn.RMSNorm(hidden, eps=1e-04, dtype=torch.float32),
                    act_cls(),
                ]
            )
            inp = hidden
        self._trunk = nn.Sequential(*blocks)
        self._gate = nn.Linear(hidden, deter, bias=True)
        self._cand = nn.Linear(hidden, deter, bias=True)
    ...
    def forward(self, stoch, deter, action):
        ...
        x = self._trunk(x)
        gate = torch.sigmoid(self._gate(x))
        cand = torch.tanh(self._cand(x))
        return gate * cand + (1.0 - gate) * deter


class TransformerBackbone(nn.Module):
    """Token-level transition using a tiny Transformer encoder."""
    ...
    def __init__(self, deter, stoch, act_dim, hidden, act="SiLU", layers=2):
        ...
        enc_layer = nn.TransformerEncoderLayer(
            d_model=deter,
            nhead=8,
            dim_feedforward=max(4 * deter, 512),
            dropout=0.0,
            activation="gelu",
            batch_first=True,
            norm_first=True,
        )
        self._enc = nn.TransformerEncoder(enc_layer, num_layers=int(layers))
    ...
    def forward(self, stoch, deter, action):
        ...
        # Two-token sequence: previous deterministic state + input token.
        seq = torch.stack([deter, token_inp], dim=1)
        out = self._enc(seq)
        return self._norm(out[:, 0] + out[:, 1])
...
class RSSM(nn.Module):
    def __init__(self, config, embed_size, act_dim):
        ...
        if self._backbone == "gru":
            ...
            self._deter_net = SSMBackbone(
                self._deter, self.flat_stoch, act_dim, self._hidden, act=config.act, layers=max(self._dyn_layers, 2)
            )
        elif self._backbone == "s5":
            self._deter_net = SSMBackbone(
                self._deter, self.flat_stoch, act_dim, self._hidden, act=config.act, layers=max(self._dyn_layers, 3)
            )
        elif self._backbone == "mamba2":
            self._deter_net = SSMBackbone(
                self._deter, self.flat_stoch, act_dim, self._hidden, act=config.act, layers=max(self._dyn_layers, 4)
            )
